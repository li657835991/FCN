...
model = load_model('/usr/local/workspace/keras-fcn/voc2011/tmp/fcn_vgg16_weights.h5',
...
Y = dataload.load_img(dataload.filenames[:])
y = datagen.standardize(x)
N = x[np.newaxis, ...]
label = dataload.load_seg(dataload.filenames)
label = np.squeeze(label, axis=-1).astype('int')
y_enc = np.eye(21)[label]
y_true = y_enc[np.newaxis, ...]
result = model.evaluate(N, y_true)
